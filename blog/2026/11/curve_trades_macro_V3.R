...
# VERSION 3 - Fixed regex escape in gsub
# =============================================================================
...
pkgs <- c("tidyverse", "lubridate", "zoo", "xts", "RcppRoll", "ggplot2", 
          "patchwork", "scales", "httr", "jsonlite", "imfr", 
...
fred_map <- list(USD = fred_USD, EUR = fred_EUR, JPY = fred_JPY, GBP = fred_GBR)
...
get_oecd_csv <- function(dataset, filter_str, start_year = 2000) {
  ...
  tryCatch({
    ...
    # FIX: Use fixed = TRUE to treat dot as literal, not regex metacharacter
